    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('manager.get.screen-ip',$sid)}}",
                    // "data":function (d){
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function ( data, type, row, meta ) {
                            ...
                            if(data == 1){
                                ...
                                output +=  '<span class="badge bg-secondary">Auto</span>' ;
                            }
                            return output;
                        }
                    },
                    {
                        ...
                        render: function ( data, type, row, meta ) {
                            console.log(row.sid);
                            let URL = "{{ route('manager.screen.screen-ip.show', [':sid',':id']) }}";
                            URL = URL.replace(':sid', row.sid);
                            URL = URL.replace(':id', row.id);

                            let ACTIVITY = "{{ route('manager.get.screen-ip-activity', [':sid',':id']) }}";
                            ACTIVITY = ACTIVITY.replace(':sid', row.sid);
                            ACTIVITY = ACTIVITY.replace(':id', row.id);



                            return '<div class="d-flex">' +
                                        @can('manager_attendance_screen-ip-show')
                                            '<a class="me-1" href="'+URL+'"><span class="badge bg-success text-dark">Show</span>' +
                                        @endcan
                                        @can('manager_attendance_screen-ip-edit')
                                        ...
                                        @endcan
                                        @can('manager_attendance_screen-ip-delete')
