        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-toolbar d-none d-md-block" role="toolbar" aria-label="Toolbar with buttons">
                    @can('manager_attendance_attendance-pg-attendance-sync')
                        <a href="{{(!empty($pg_url) && isset($pg_url)) ? $pg_url : ''}}" class="btn btn-sm btn-success">{{(!empty($pg_url_text) && isset($pg_url_text)) ? $pg_url_text : ''}}</a>
                    @endcan
                    @can('manager_attendance_attendance-create')
                        <a href="{{(!empty($url) && isset($url)) ? $url : ''}}" class="btn btn-sm btn-primary">{{(!empty($url_text) && isset($url_text)) ? $url_text : ''}}</a>
                    @endcan
                    @can('manager_attendance_attendance-activity-log')
                        <a href="{{(!empty($trash) && isset($trash)) ? $trash : ''}}" class="btn btn-sm btn-danger">{{(!empty($trash_text) && isset($trash_text)) ? $trash_text : ''}}</a>
                    @endcan
                    @can('manager_attendance_attendance-erp-attendance-sync')
                        <a href="{{(!empty($erp_url) && isset($erp_url)) ? $erp_url : ''}}" class="btn btn-sm btn-warning">{{(!empty($erp_url_text) && isset($erp_url_text)) ? $erp_url_text : ''}}</a>
                    @endcan
                        @can('manager_attendance_attendance-delete-all')
                            {{--                                        <a href="{{(!empty($delete_all) && isset($delete_all)) ? $delete_all : ''}}" class="btn btn-sm btn-danger">{{(!empty($delete_all_text) && isset($delete_all_text)) ? $delete_all_text : ''}}</a>--}}
                        ...
                        <div class="col-12 mb-4">
                            <fieldset class="reset-this redo-fieldset">
                                ...
                                <div class="row gy-2 gx-3 align-items-center    ">
                                    <div class="col-3">
                                        <label class="mb-1">Sync Status</label>
                                        <select class="select2-options-sync-status form-control" id="sync_status">
                                            <option value="">All</option>
                                            <option value="1">Sync</option>
                                            <option value="0">UnSync</option>
                                        </select>
                                    </div>
                                    <div class="col-6">
...
@push('footer-scripts')
    ...
    <script>
        $(document).ready(function () {
            //Select Sync Status
            $('.select2-options-sync-status').select2({
                theme: "bootstrap5",
                placeholder: 'All',
                allowClear: true
            });
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('manager.get.attendance')}}",
                    "data":function (d){
                        d.sync_status = document.getElementById('sync_status').value;
                ...
                columnDefs:  [
                    ...
                    },
                    {
                        ...
                        render: function ( data, type, row, meta ) {
                            ...
                            if(data == 1){
                                output +=  '<span class="badge bg-success">In</span>' ;
                            }
                            else{
                                output +=  '<span class="badge bg-danger">Out</span>' ;
                            }
                    ...
                    },
                    {
                        ...
                        render: function ( data, type, row, meta ) {
                            ...
                            if(data == 1){
                                output +=  '<span class="badge bg-success">Sync</span>' ;
                            }
                            else{
                                output +=  '<span class="badge bg-danger">UnSync</span>' ;
                            }
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function ( data, type, row, meta ) {
                            let URL = "{{ route('manager.attendance.show', ':id') }}";
                            URL = URL.replace(':id', row.id);
                            let ACTIVITY = "{{ route('manager.get.attendance-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);
                            return '<div class="d-flex">' +
                                    @can('manager_attendance_attendance-show')
                                    '<a class="me-1" href="'+URL+'"><span class="badge bg-success text-dark">Show</span>' +
                                    @endcan
                                    @can('manager_attendance_attendance-edit')
                                    ...
                                    @endcan
                                    @can('manager_attendance_attendance-delete')
