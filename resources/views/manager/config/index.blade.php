            ...
            $('#indextable').DataTable({
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function ( data, type, row, meta ) {
                            let URL = "{{ route('manager.config.show', ':id') }}";
                            URL = URL.replace(':id', row.id);
                            let ACTIVITY = "{{ route('manager.get.config-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);
                            return '<div class="d-flex">' +
                                        @can('manager_attendance_config-show')
                                            '<a class="me-1" href="'+URL+'"><span class="badge bg-success text-dark">Show</span>' +
                                        @endcan
                                        @can('manager_attendance_config-edit')
                                        ...
                                        @endcan
                                        @can('manager_attendance_config-delete')
